...
# Default youtube-dl options to use for downloading each URL
youtube_dl = ["youtube-dl", "-t", "-q", "-c"]
...
def download(request_path, download_path):
    ...
    # Build a list of youtube-dl jobs, one for each URL in our input file(s).
    ...
    logging.info("%d youtube-dl jobs queued" % len(jobs))
